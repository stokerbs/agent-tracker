...
interface Props {
  phone: string;
  next: string;
  /** Where the resend link points — defaults to /login */
  resendHref?: string;
}
...
export function OtpVerifyForm({ phone, next, resendHref = "/login" }: Props) {
  ...
  const resendUrl = `${resendHref}${phone ? `?phone=${encodeURIComponent(phone)}` : ""}`;
