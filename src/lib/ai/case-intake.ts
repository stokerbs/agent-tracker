...
function buildContent(files: IntakeFilePart[]): ContentBlock[] {
  ...
  blocks.push({
    type: "text",
    text: "Extract all case intelligence from the files above using the extract_case_intake tool. Facts only — no inference. Write all free-text output in Thai by default, preserving names, addresses, plates, phone numbers, emails and URLs exactly as written.",
  });
  return blocks;
}
