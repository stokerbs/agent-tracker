...
vi.mock("@/lib/notifications", () => ({
  notifyRole: vi.fn(),
  notificationLinks: { recruitment: () => "/recruitment" },
}));
...
describe("POST /api/marketing/careers", () => {
  it("200 and inserts on a valid application", async () => {
    const s = svc();
  ...
  it("stores a valid email when provided", async () => {
    const s = svc();
    vi.mocked(createServiceClient).mockReturnValue(s.client as never);
    const res = await POST(req({ ...valid, email: "apply@example.com" }));
    expect(res.status).toBe(200);
    expect(s.insert).toHaveBeenCalledWith(expect.objectContaining({ email: "apply@example.com" }));
  });

  it("stores email/position/experience as null when omitted", async () => {
    const s = svc();
    vi.mocked(createServiceClient).mockReturnValue(s.client as never);
    const { position, experience, ...bare } = valid;
    void position; void experience;
    await POST(req(bare));
    expect(s.insert).toHaveBeenCalledWith(
      expect.objectContaining({ email: null, position: null, experience: null }),
    );
  });
