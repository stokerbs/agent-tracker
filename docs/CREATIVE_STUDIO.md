...
Known gaps, measured rather than guessed. `NOT_A_NAME` matches a prefix, so every token in it costs the names that start with it: การ, จะ, ต้อง and ทีม stay (they earn their place against real false positives) and take การุณ, จะเด็ด, ต้องตา and ทีม with them; the thirteen measured as earning nothing (with เลขที่, เลขคดี, ตรงกับ and ตรงกัน added back as whole phrases, which is what those prefixes were really for) (ตาม, จาก, เป็น, ครบ, ถูก, ผิด, ยัง, เต็ม, จริง, เลข, รูป, ตรง, and แพ็ narrowed to แพ็ก) are gone — those four were costing เลขา, ตรงใจ, แพ็ท and แพ็ตตี้ while เลขบัตร, รูปถ่าย and แพ็ก already covered what they were for. A form dump — "ชื่อ นามสกุล สมชาย ใจดี" — is not detected: the field label stops the scan rather than being skipped, and every attempt to skip it brought back worse false positives. ชื่อจริง stays a compound, so a person actually called จริงใจ is missed. A name with no cue at all ("จำไม่ได้ว่าสมชายหรือธนวัฒน์") is not caught by any rule here, on main or now. `ชื่อ<slot word>คือ<name>` is silent for all 45 slot words: right for the field labels, wrong for พยาน, which in this business names a person — kept because the label sense is the common one in the rows. The compensating controls for all of these are the owner denylist (high severity) and the AI privacy pass, which may only make a verdict stricter. `redactForInbox` replaces the *name inside* a finding rather than the whole finding where it can: a name rule matches the phrase around the name ("ชื่อเล่นของแฟนผม บอย อยู่บางนา"), and blanking all of that would take the question this inbox exists to mine. It does not require a word boundary after the name — Thai runs the next word straight into it, and demanding one skipped most real names and stored them raw (61% in one measured build, against 29% for blanking everything). Three attempts to *infer* whether the right word was picked ("nothing changed", "a cue is still ahead", "we stopped mid-word") each missed a new shape, because a wrong pick is worse than an obvious miss: the row keeps the name and gains a `[ชื่อ]` that reads as redacted. So the pick is now **checked instead of inferred** — a pick from the particle/grammar list is a failure by definition, and the finding with the pick removed is re-read by the same scanner; if it still sees a name, the whole finding is blanked. Measured across 1,880 generated messages: no name readable and no misleading token in any class, against 33–100% leakage per class on the previous behaviour.

The cost is the enquiry, and it is real. When the scan's gap crosses the spaces it can run past the name and over the question ("ชื่อของสามี สมชาย ครับ ช่วยดูให้ได้ไหม" is one finding), and blanking it takes both — on the glued-label shapes only 2.1% of messages kept any of the customer's own words after the name. Names never leak is the owner's rule, so that is the right way round, but the fix that buys the question back sits in the scanner, not here: the gap should stop before a politeness particle, so the finding ends at the name instead of swallowing the sentence. Until then the mining layer sees fewer complete questions from name-bearing messages. Messages with no personal name in them are untouched (0/18 measured).
