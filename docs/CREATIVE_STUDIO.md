...
Known gaps, measured rather than guessed. `NOT_A_NAME` matches a prefix, so every token in it costs the names that start with it: การ, จะ, ต้อง and ทีม stay (they earn their place against real false positives) and take การุณ, จะเด็ด, ต้องตา and ทีม with them; the thirteen measured as earning nothing (with เลขที่, เลขคดี, ตรงกับ and ตรงกัน added back as whole phrases, which is what those prefixes were really for) (ตาม, จาก, เป็น, ครบ, ถูก, ผิด, ยัง, เต็ม, จริง, เลข, รูป, ตรง, and แพ็ narrowed to แพ็ก) are gone — those four were costing เลขา, ตรงใจ, แพ็ท and แพ็ตตี้ while เลขบัตร, รูปถ่าย and แพ็ก already covered what they were for. A form dump — "ชื่อ นามสกุล สมชาย ใจดี" — is not detected: the field label stops the scan rather than being skipped, and every attempt to skip it brought back worse false positives. ชื่อจริง stays a compound, so a person actually called จริงใจ is missed. A name with no cue at all ("จำไม่ได้ว่าสมชายหรือธนวัฒน์") is not caught by any rule here, on main or now. `ชื่อ<slot word>คือ<name>` is silent for all 45 slot words: right for the field labels, wrong for พยาน, which in this business names a person — kept because the label sense is the common one in the rows. The compensating controls for all of these are the owner denylist (high severity) and the AI privacy pass, which may only make a verdict stricter. **Where the name is (2026-09-26).** Four builds of the inbox redaction tried to work out which token inside a name finding was the name, and each one shipped a row that kept the name and gained a `[ชื่อ]` reading as redacted — worse than an obvious miss, because a person or a later step believes it. The gates measured the last of them at 100% leakage on the commonest shape of all. The cause was never the inbox: `NAME_INTRO_RE`'s gap crossed spaces and particles, so its finding ran past the name and over the question ("ชื่อของสามี สมชาย ครับ ช่วยดูให้ได้ไหม" was one finding ending inside ไหม) and nothing in it said where the name was. Two changes fix it at the source and one follows:

- the gap stops at a **particle after a space** and the name slot refuses a particle outright, so a finding now ENDS at the name. Particles cannot be tested as bare prefixes — เจ้าหนี้ contains จ้า and มะนาว contains นะ, the same trap that `NOT_A_NAME` pays for with การุณ and จะเด็ด — so the gap test requires the space and the slot test requires the particle to be the whole slot, which keeps คะนอง a name.
- the inbox splits the rules by whether their match can hide the name. `NAME_CUE_RE`, the titles and `KHUN_NAME_RE` put the name immediately after the cue, so there the name alone is replaced and the customer's question survives. A `ชื่อของ/ชื่อใน/ชื่อที่` finding can hold the name anywhere in its gap, so it goes whole, plus a following token unless that token is a particle — a Thai full name is two tokens and the finding ends at the first. Replacements take the rest of the word with them, since the ten-letter slot can cut one in half and leave an orphan syllable.
- identifiers are redacted first and names are then read off the text as it stands, because a name finding is wide enough to overlap a plate or a phone number: redacting it first erased the text the plate finding was waiting to match and the row kept the digits.

Measured on 6,048 generated messages across nine shapes, including the compound tails (`นะครับ`, `ด้วยครับ`, `อยู่บางนา`) whose absence from an earlier corpus is what hid the last regression: a name is readable in 168 rows against 4,699 on main, **no row carries a misleading token** (702 on main), no orphan syllables, and 1,463 rows keep the customer's question where main kept none. All 168 are the one shape no rule sees.

**Known gap: a label glued to the name.** "ชื่อของลูกค้าสมชาย ครับ" has no space between the label and the name, so the slot can only land on the particle, which it now refuses — the scan reports nothing and the message is stored as written, exactly as on main. Opening `ชื่อของ/ชื่อใน` wide enough to catch it flagged 14 of 15 real customer questions in measurement, which is not a trade worth making; a false positive here holds a finished clip, which cost three weeks of silence in September. `ชื่อเล่น` is the exception and has its own rule: a nickname only ever belongs to a person, so the glued form is safe there, and it is a shape the real rows write constantly (40 of 100 such messages stored the name on main, none now).
