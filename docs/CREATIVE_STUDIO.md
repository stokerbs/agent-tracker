...
**Whose name (2026-09-25).** The rule added on 2026-09-15 to catch "ชื่อของลูกค้า สมชาย" took any text between the particle and the name, so it read "ลูกค้าถือ**ชื่อที่ไม่แน่ใจว่า**ใช่หรือเปล่า" as an introduction — and held a finished clip for owner review on 2026-09-24 while the AI reviewer, in the same record, wrote that the scanner had got it wrong. Nobody was watching the queue, so nothing posted that week. The rule now requires a word that says *whose* name it is (`NAME_LABEL`: ลูกค้า, ผู้เสียหาย, เป้าหมาย, สามี, บัตรประชาชน, ใบสมัคร…), and the cue rule only takes an unglued name when a title follows it — a space after ชื่อ is how Thai writes a list of fields ("ชื่อ ที่อยู่เก่า และเบอร์โทร" is three column headings, not a person).
