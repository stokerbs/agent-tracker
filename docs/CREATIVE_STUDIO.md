...
A name the rule reports is redacted at **every** mention in the message, not only inside the finding that reported it — naming the target and then talking about them ("ชื่อของลูกค้าคือ สมชาย ครับ สมชายหายไป 3 วัน") is the most natural way to write one of these messages, and both gates found that redacting the first mention only left the name in the row beside a token saying it was gone. The first token of a full name is searched too, since the later mention is usually the first name alone. The token after a name goes with it unless it is a particle or a grammar word, because a Thai surname alone identifies a family and reads as redacted when a `[ชื่อ]` sits in front of it; only the `คือ` rule can report that second token itself, since extending the others read "นางสาวสมหญิง ทำงานที่นั่น" as a full name. When the next word is not a surname, the row loses one word of the question — the cheaper way to be wrong in a store of customer PII.
