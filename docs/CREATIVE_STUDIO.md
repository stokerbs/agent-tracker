...
- the inbox redacts in one pass over spans computed on the original text. Doing it finding by finding let a wide name finding erase the text a plate finding was still waiting to match (the row kept the digits), and doing identifiers first let the name pass eat a token that had just been inserted. Spans extend to the end of the word, because a slot stops at ten letters and Thai writes no space inside a word.
