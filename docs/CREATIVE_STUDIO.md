...
**Whose name (2026-09-25).** The rule added on 2026-09-15 to catch "ชื่อของลูกค้า สมชาย" took any text between the particle and the name, so it read "ลูกค้าถือ**ชื่อที่ไม่แน่ใจว่า**ใช่หรือเปล่า" as an introduction — and held a finished clip for owner review on 2026-09-24 while the AI reviewer, in the same record, wrote that the scanner had got it wrong. Nobody was watching the queue, so nothing posted that week. The first attempt at a fix required a word saying *whose* name it is, and the security gate measured what that cost: Thai writes "whose" **in front of** ชื่อ at least as often as behind it (ลูกค้าชื่อ สมชาย, พยานชื่อ สมหญิง), and the list of whose-words cannot be closed — every kind of case brings a new one (คนหาย, ผู้เช่า, เจ้าหนี้, ทายาท, ชู้). So the rule enumerates the other side instead: the **name slot**. `NOT_A_NAME` is the field labels and grammar words that are never a person (ที่อยู่, นามสกุล, เบอร์, อายุ, ที่, ไม่, การ…), a gap that says the name is unknown (ไม่แน่ใจ, ไม่ตรง, สะกด) is not an introduction, and on the spaced form a field word in the gap means the sentence is listing fields rather than naming anyone. A sentence that hedges (`HEDGE`: ไม่แน่ใจ, ไม่ตรง, จำไม่ได้, ไม่รู้, สงสัย, พิสูจน์) is talking about a name nobody is sure of — but only before ว่า; before คือ a name really does follow ("ชื่อที่ไม่ตรงกับบัตรคือสมชาย"), which is where the two gates disagreed until the guard was split by terminator. `NOT_A_CUE` keeps ชื่อเสียง/ชื่อบัญชี out of both rules — "รักษาชื่อเสียงของลูกค้า" is this brand's own copy — and ชื่อเล่น takes the same particles as ชื่อ, since a nickname is full PII and the rows write "ชื่อเล่นของเป้าหมายคือ…" constantly.

Known gaps, measured rather than guessed. `NOT_A_NAME` matches a prefix, so every token in it costs the names that start with it: การ, จะ, ต้อง and ทีม stay (they earn their place against real false positives) and take การุณ, จะเด็ด, ต้องตา and ทีม with them; the nine tokens that were measured as earning nothing (ตาม, จาก, เป็น, ครบ, ถูก, ผิด, ยัง, เต็ม, จริง) are gone. A form dump — "ชื่อ นามสกุล สมชาย ใจดี" — is not detected: the field label stops the scan rather than being skipped, and every attempt to skip it brought back worse false positives. ชื่อจริง stays a compound, so a person actually called จริงใจ is missed. The compensating controls for all of these are the owner denylist (high severity) and the AI privacy pass, which may only make a verdict stricter.
