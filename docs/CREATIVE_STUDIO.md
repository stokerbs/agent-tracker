...
- Captions: one line at a time (`captionChunks`, 14 target / 30 max graphemes, Sarabun Bold 92 with a heavy outline) at y 1230, popping in, with numbers and `EMPHASIS_WORDS` in yellow. A highlight never starts right after a leading vowel or ends before a combining mark. The 30-grapheme ceiling is deliberate: a shorter one makes the breaker split more phrases on ICU word boundaries, and ICU over-splits compounds (หลัก|ฐาน, ลงพื้น|ที่).
