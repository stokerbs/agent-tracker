...
Three known gaps stay, each measured rather than guessed. **A label glued to the name** ("ชื่อของลูกค้าสมชาย ครับ", and the same with no space at all) has no separator, so no rule can say which token is the name and **none of them flag it at all** — this is invisible to the publish gate too, not only to the inbox, and the message is stored as written, exactly as on main. It is 1,568 of the 1,582 remaining leaks. Opening `ชื่อของ/ชื่อใน` wide enough to catch it flagged 14 of 15 real customer questions, and a `ชื่อเล่น`-only version still swallowed 8 of 15; a false positive here holds a finished clip, which cost three weeks of silence in September. **A surname that contains a particle or question word as a syllable** (นะวะมันดา) is read as speech and stays — 1 of 18 measured surname shapes, against 18 on main. **A name that is literally a particle** ("ชื่อของลูกค้าคือ นะ ครับ") is silenced by the slot rule, accepted because a person called นะ or คะ is not a real case while เจ้าหนี้, มะนาว and คะนอง are — and each of those was silenced in measurement by a particle list that was not anchored to a space.
