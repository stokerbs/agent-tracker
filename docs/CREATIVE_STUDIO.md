...
Three known gaps stay, each measured rather than guessed. **A label glued to the name** ("ชื่อของลูกค้าสมชาย ครับ", and the same with no space at all) has no separator, so no rule can say which token is the name and none of them flag it — the message is stored as written, as on main. Opening `ชื่อของ/ชื่อใน` wide enough to catch it flagged 14 of 15 real customer questions, and a `ชื่อเล่น`-only version still swallowed 8 of 15; a false positive here holds a finished clip, which cost three weeks of silence in September. **A name that is literally a particle** (`ชื่อของลูกค้าคือ นะ ครับ`) is silenced by the slot rule — 2 of the 7,614 rows, accepted because a person called นะ or คะ is not a real case while เจ้าหนี้ and คะนอง are. **A surname longer than twelve letters** after a captured first name stays, since the second token is bounded to keep it from swallowing a clause.
